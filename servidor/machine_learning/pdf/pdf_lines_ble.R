...
#creating color range (it's battery problem)
mycolors <- colorRampPalette(c("green", "red"))
mycolors <- mycolors(max(data[,2])+1)
...
plot(data[1,1],1,xlab='Time',xlim=c(0,24),ylim=c(trunc(as.numeric(difftime(data[1,1], init,units="days"))),trunc(as.numeric(difftime(data[nrow(data),1], init,units="days")))+2),ylab='',
  col=mycolors[data[1,2]+1],pch=19,cex=cex,xaxt="n",yaxt="n")
...
for (i in 2:nrow(data)){
  ...
  # get color
  # modification in code because it's BLE problem
  if (data[i,2] < 1){
    color <- "white"
  }else{
