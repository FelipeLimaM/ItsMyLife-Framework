...
mycolors <- c(257,123,28,95,451,102,595,74,636,644,477,47,90,568,643,94,650,54,485,130,640,116,614,58,159,321,619)
...
plot(data[1,1],1,xlab='Time',xlim=c(0,24),ylim=c(trunc(as.numeric(difftime(data[1,1], init,units="days"))),trunc(as.numeric(difftime(data[nrow(data),1], init,units="days")))+2),ylab='',
  col=mycolors[data[1,2]],pch=19,cex=cex,xaxt="n",yaxt="n")
...
current_day <- c(1)
current_color <- c(mycolors[data[2,2]+1])
X <- c(current_day)
...
for (i in 2:nrow(data)){
  ...
  if(color != current_color){
    #End a line
    X <- c(X, current_day)
    if (time < c(as.numeric(difftime(data[i-1,1], init,units="hours"))%%24)){
      Y <- c(Y, c(as.numeric(difftime(data[i,1], init,units="hours"))%%24)) # TIME
    }else{
      Y <- c(Y, c(as.numeric(difftime(data[i-1,1], init,units="hours"))%%24)) # TIME
    }
    current_color <- color
